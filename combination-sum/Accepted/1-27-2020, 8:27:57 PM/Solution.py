...
class Solution:
    def permutate(self, c: List[int], pos: int, target: int, answer: List[str]) -> None:
        while pos < len(c):
            val: int = target - c[pos]
            if val == 0:
                # Found answer, add it to output list
                answer.append(c[pos])
                self.output.append(list(answer))
                ...
                # Try to recurse with same value from candidates list as val > 0
                answer.append(c[pos])
                self.permutate(c, pos, val, answer)
                del answer[len(answer) - 1]
    ...
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        ...
        self.permutate(candidates, 0, target, [])
        return self.output
#     def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
#         results = []
#         if(len(candidates) == 0):
